...
func (c *Command) isValidConfig(config, cmdConfig *Config) bool {
	...
	if config.Server.DynamicPortRangeMin > 0 &&
		config.Server.DynamicPortRangeMax > 0 &&
		config.Server.DynamicPortRangeMin >= config.Server.DynamicPortRangeMax {
...
func (c *Command) Run(args []string) int {
	...
	if config.Server.DynamicPortRangeMin > 0 && config.Server.DynamicPortRangeMax > 0 {
		structs.SetDynamicPortRange(
			structs.PortRange{
				Min: config.Server.DynamicPortRangeMin,
				Max: config.Server.DynamicPortRangeMax,
			},
		)
	}
